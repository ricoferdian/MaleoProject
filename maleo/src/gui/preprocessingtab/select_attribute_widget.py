...
class SelectAttributeWidget(QWidget):
    ...
    def update_widget(self, values):
        if len(values):
            ...
            self._change_label(self.selectedData)
            self._change_table(self.selectedData)

    def _change_label(self, data):
    ...
    def _change_table(self, data):
        ...
        if self.dataType == self.dataTypeCheck.getType().Numeric:
            ...
            self._draw_table(tableData, 4, 2)
            ...
            self._draw_table(tableData, len(uniqueLabels), 3)
        else:
            print("Unknown")
